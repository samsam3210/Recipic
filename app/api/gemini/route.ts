...
import { generateTextWithGemini } from "@/lib/utils/gemini-api-wrapper"
...
5.  **난이도 측정**: 난이도는 필수 항목입니다. 조리 단계나 재료의 복잡성을 기반으로 측정하되, 정보가 부족한 경우 레시피 제목을 기반으로라도 유추하여 반드시 '쉬움', '보통', '어려움' 중 하나의 값을 제공해주세요. '측정불가'나 null 값은 허용되지 않습니다.
6.  **조리시간 측정**: 조리시간(cookingTimeMinutes)도 필수 항목입니다. 영상 스크립트 내용을 기반으로 정확한 조리시간을 추출하되, 할루시네이션은 엄격히 금지합니다. 스크립트에서 직접적인 시간 정보를 찾을 수 없는 경우, 조리 단계의 개수와 복잡도, 레시피 제목, 재료 준비 과정 등을 종합적으로 고려하여 합리적인 조리시간을 분 단위로 산출해주세요. null이나 0값은 허용되지 않으며, 반드시 숫자값을 분으로 제공해야 합니다.
7.  **누락된 레시피**: 만약 레시피를 찾을 수 없다면, \`recipeName\`을 \`null\`로, \`noRecipeFoundMessage\`에 적절한 안내 문구를 넣어주세요.
